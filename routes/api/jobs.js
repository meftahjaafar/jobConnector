    ...
    async (req, res) => {
      ...
      try {
       ...
       if (company){
         ...
         return res.status(401).json({msg: 'You are not authorized to post a job !'})
       }
...
router.put('/job/close/:id', auth, async (req, res) => {
    ...
    if( company){
       ...
       // @check if job is posted by this current HR recruiter
       if (jobPost.postedby.toString() === req.user.id){
         await Job.findByIdAndUpdate({ _id: req.params.id }, { isactive: false })
...
router.get('/', auth, async (req, res) => {
    try {
        ...
        if (!jobposts){
           return  res.status(404).json({msg:'There are no Job posts to display !'});
        }
        res.json(jobposts)
    } catch (error) {
        console.error(error.message);
...
router.put('/applyjob/:id', auth, async (req,res) =>{
    try {
      ...
      if (jobPost.appliedby.filter(offer => offer.user.toString() === req.user.id).length > 0){
        return res.status(401).json({msg:'You have been applied this Job post!'})
      }
      const newApp = {
        user : req.user.id,
        name : req.user.name,
...
router.get('/:company_id/jobposts', auth, async (req, res) => {
    try {
        let jobPosts = await Job.find({company: req.params.company_id}).populate('companies').select('-appliedby')
      ...
      try {
       ...
       if (company){
        ...
        if (jobPost) {
         ...
         return res.status(401).json({msg: 'You are not authorized to update a job !'})
       }
